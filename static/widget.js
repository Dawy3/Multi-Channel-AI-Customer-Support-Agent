(function () {
  ...
  const DEFAULTS = {
    ...
    projectId: 1,   // which RAG project/collection to query
  ...
  function saveHistory(uiMessages, chatHistory) {
    try {
      const existing = loadHistory();
      const createdAt = (existing && existing.createdAt) || Date.now();
      localStorage.setItem(STORAGE_KEY, JSON.stringify({
        messages: uiMessages,     // rendered bubbles, for UI restore
        chatHistory: chatHistory, // API format [{ role, content }], for context
        createdAt: createdAt,
      }));
    } catch (e) {}
  }
  ...
  function initWidget() {
    ...
    for (const s of scripts) {
      if (s.src.includes("widget.js")) {
        ...
        if (s.dataset.projectId) cfg.projectId = parseInt(s.dataset.projectId, 10) || cfg.projectId;
        if (s.dataset.limit) cfg.limit = parseInt(s.dataset.limit, 10) || cfg.limit;
        break;
      }
    }
    ...
    let chatHistory = [];   // conversation in API format: [{ role, content }]
    let uiMessages = [];    // rendered bubbles, for restoring on reload: [{ role, text }]
    ...
    // ── Send ──────────────────────────────────────────────────────
    // Calls the project's RAG endpoint:
    //   POST {apiUrl}/api/v1/nlp/index/answer/{projectId}
    //   body  -> { text, limit, chat_history: [{ role, content }] }
    //   reply -> { signal, answer, retrieved_documets, chat_history }
    async function send() {
      ...
      addUserMessage(text);
      uiMessages.push({ role: "user", text: text });
      input.value = "";
      input.style.height = "auto";
      ...
      try {
        const response = await fetch(
          cfg.apiUrl + "/api/v1/nlp/index/answer/" + cfg.projectId,
          {
            method: "POST",
    ...
    // ── Restore previous conversation (within TTL) so a reload keeps context ──
    (function restore() {
